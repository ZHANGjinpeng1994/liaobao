...
import com.liaobao.db.dao.DaoMaster;
import com.liaobao.db.dao.DaoSession;
import com.liaobao.db.dao.NewFriendDao;
...
public class NewFriendManager {
    ...
    private List<NewFriend> getNoVerifyNewFriend(){
        NewFriendDao dao =  openReadableDb().getNewFriendDao();
        return dao.queryBuilder().where(NewFriendDao.Properties.Status.eq(APP.STATUS_VERIFY_NONE))
                .build().list();
    }
    ...
    public void updateBatchStatus(){
        ...
        if(infos!=null && infos.size()>0){
            ...
            for (int i = 0; i < size; i++) {
                ...
                msg.setStatus(APP.STATUS_VERIFY_READED);
                all.add(msg);
            }
